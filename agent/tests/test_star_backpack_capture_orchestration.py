...
def _params(directory):
    return {
        ...
        "allow_main_only_transport_smoke": False,
        "game_version": "如鸢",
        "debug_dir": directory,
...
class StarBackpackCaptureOrchestrationTests(unittest.TestCase):
    ...
    def test_production_params_reject_b4_transport_smoke(self):
        with tempfile.TemporaryDirectory() as directory:
            params = _params(directory)
            params["allow_main_only_transport_smoke"] = True
            with self.assertRaisesRegex(ValueError, "transport smoke"):
                parse_params(params)
    ...
    def test_pipeline_uses_click_nodes_without_ocr_and_exposes_full_batch_transport(self):
        ...
        self.assertFalse(params["allow_main_only_transport_smoke"])
        self.assertEqual(
            json.loads((AGENT_ROOT.parent / "assets" / "interface.json").read_text(encoding="utf-8"))["task"][3]["option"],
            ["同步星石至YuanHub"],
        )
